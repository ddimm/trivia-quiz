...
const parrot =
  "https://appstickers-cdn.appadvice.com/1158454115/819397787/4b86cdd9a66526c025daf6d6c98e82e1-0.gif";
        ...
        <ThemeProvider theme={themes.default}>
          ...
          >
            {questions.map((value, index, array) => {
              return (
                <Question {...value} questIndex={index + 1} key={index + 1} />
              );
            })}
          </div>
        </ThemeProvider>
...
function Question({
  ...
  const [questionList, setQuestionList] = useState(
    [correct_answer, ...incorrect_answers].sort((a, b) => {
      if (a === "True" && b === "False") {
        return -1;
      }
  ...
  for (let i = 0; i < parrotCount; i++) {
    parrots.push(
      <img className="Parrot" alt="this is a parrot" src={parrot}></img>
    );
  }
  useEffect(() => {
    ...
    return (
      <Window className="Window">
        ...
        <WindowContent>
          ...
          <h3 style={{ color: "red", marginTop: 2, marginBottom: 10 }}>
            You are incorrect
          </h3>
        </WindowContent>
      </Window>
